...
- Pick your licence class from the dropdown (US options are listed first;
  US General is the default). The band checkboxes below it update to show
  exactly what that licence is allowed to use on each band, including any
...
- **Stop** finishes the in-progress step, sends the tuner a stop command,
  and halts.
...
This has been built directly from the FTX-1 CAT Operation Reference Manual
(`refs/FTX-1_CAT_OM_ENG_2508-C.pdf`) but hasn't been run against real
hardware. Before sweeping a whole band unattended, test with a single narrow
...
- Every step is a real transmission. Keep power low (10-20 W is plenty for
  tuning). Pick the licence class that actually matches what you hold -
  the US entries are exact, but the Canada/Europe/Japan/Central & South
  America entries are approximate reference allocations, not verified
  per-class tables (see [band-privileges.md](band-privileges.md)). If in
  doubt, use a custom range you've confirmed yourself rather than trusting
  a `_ref` entry.
...
- If you hit **Stop**, the app still sends a tuner-stop command and closes
  the serial port in the background, but nothing replaces being able to
  reach the radio's power switch.
...
- `bandplans.tsv` — licence classes and their legal frequency ranges, user
